...
class ConditionService:
    ...
    async def get_eligible_tenants(
        self, conditions: dict[str, Any], db: Session
    ) -> list[Tenant]:
        """
        ...
        try:
            ...
            logger.info(
                f"Found {len(eligible_tenants)} eligible tenants based on conditions"
            )

            return eligible_tenants

        except Exception as e:
            logger.error(f"Failed to get eligible tenants: {e!s}")
            return []

    def _apply_conditions(self, query, conditions: dict[str, Any], db: Session):
        """
    ...
    def _build_condition_filter(self, rule: dict[str, Any], db: Session):
        ...
        try:
            ...
            logger.error(f"Failed to build condition filter: {e!s}")
            return None
    ...
    def evaluate_tenant_conditions(
        self, tenant: Tenant, conditions: dict[str, Any], db: Session
    ) -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Failed to evaluate tenant conditions: {e!s}")
            return False
    ...
    def _evaluate_single_rule(
        self, tenant: Tenant, rule: dict[str, Any], db: Session
    ) -> bool:
        """
        ...
        try:
            ...
            logger.error(f"Failed to evaluate single rule: {e!s}")
            return True  # Default to true to avoid blocking

    def _apply_operator(
        self, tenant_value: Any, operator: str, expected_value: Any
    ) -> bool:
        """
        ...
        try:
            if operator == "eq":
                ...
                return (
                    tenant_value in expected_value
                    if isinstance(expected_value, list)
                    else tenant_value == expected_value
                )
            elif operator == "not_in":
                return (
                    tenant_value not in expected_value
                    if isinstance(expected_value, list)
                    else tenant_value != expected_value
                )
                ...
                if isinstance(tenant_value, datetime) and isinstance(
                    expected_value, int
                ):
            ...
            logger.error(f"Failed to apply operator {operator}: {e!s}")
            return True
    ...
    def get_predefined_conditions(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "late_fees": {
                "operator": "and",
                "rules": [
                    {"field": "late_fee_applicable", "operator": "eq", "value": True}
                ],
            },
            "overdue_rent": {
                "operator": "and",
            ...
            "all_tenants": {"operator": "and", "rules": []},
        }
    ...
    async def test_conditions(
        ...
        try:
            ...
            return {
                ...
                "match_percentage": (
                    (len(eligible_tenants) / total_tenants * 100)
                    if total_tenants > 0
                    else 0
                ),
                "tenant_names": [
                    t.name for t in eligible_tenants[:10]
                ],  # First 10 names
