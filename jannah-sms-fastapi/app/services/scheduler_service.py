...
class SchedulerService:
    ...
    async def _load_existing_schedules(self) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to load existing schedules: {str(e)}")
    ...
    async def _schedule_job(self, schedule: Schedule) -> None:
        ...
        try:
            ...
            logger.info(f"Scheduled job: {schedule.name} - Next run: {job.next_run_time}")
            
        except Exception as e:
            logger.error(f"Failed to schedule job for {schedule.name}: {str(e)}")
            raise
    ...
    async def _execute_schedule(self, schedule_id: int) -> None:
        ...
        try:
            with SessionLocal() as db:
                ...
                rent_day = int(schedule.schedule_value) if schedule.schedule_value else 5
    ...
    async def _create_message_records(
        ...
        try:
            ...
            logger.error(f"Failed to create message records: {str(e)}")
            db.rollback()
    ...
    async def run_schedule_now(self, schedule_id: int, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            error_msg = f"Failed to run schedule {schedule_id}: {str(e)}"
