...
async def send_message_form(
    ...
    try:
        # Get all active tenants with opt-in status
        tenants = (
    ...
    try:
        ...
        if not tenants:
            ...
            return RedirectResponse(
                url="/messages/send?error=no_recipients", status_code=302
            )
        ...
        if send_time == "now":
            ...
            for sms_result in result["results"]:
                # Skip creating records for skipped tenants (not opted in)
                if sms_result.get("skipped"):
                    continue
            ...
            minute_final = (
                send_minute_final
                if send_minute_final is not None
                else (
                    custom_minute
                    if custom_minute is not None
                    else (send_minute if send_minute is not None else 0)
                )
            )

            if not send_date or hour_final is None:
                return RedirectResponse(
                    url="/messages/send?error=missing_schedule_info", status_code=302
                )
        ...
        logger.error(f"Send message submit error: {e!s}")
        return RedirectResponse(url="/messages/send?error=send_failed", status_code=302)


async def _get_eligible_tenants(
    recipient_type: str, selected_tenants: Optional[list], db: Session
) -> list[Tenant]:
...
def _parse_schedule_time(
    send_date: str, send_hour: int, send_minute: int = 0
) -> datetime:
