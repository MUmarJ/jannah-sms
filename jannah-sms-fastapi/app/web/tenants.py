...
async def tenants_list(
    ...
    try:
        ...
        logger.error(f"Tenants list error: {str(e)}")
        return templates.TemplateResponse(
            "error.html",
            {
                "request": request,
    ...
    try:
        ...
        if existing_tenant:
            return RedirectResponse(
                url="/tenants/add?error=phone_exists",
                status_code=302
            )
        ...
        return RedirectResponse(
            url=f"/tenants?success=tenant_added",
            status_code=302
        )
        
    except Exception as e:
        db.rollback()
        logger.error(f"Add tenant error: {str(e)}")
        return RedirectResponse(
            url="/tenants/add?error=add_failed",
            status_code=302
        )


@router.get("/{tenant_id}/edit", response_class=HTMLResponse)
async def edit_tenant_form(tenant_id: int, request: Request, db: Session = Depends(get_db)):
...
async def edit_tenant_submit(
    ...
    try:
        ...
        if phone != tenant.contact:
            ...
            if existing_tenant:
                return RedirectResponse(
                    url=f"/tenants/{tenant_id}/edit?error=phone_exists",
                    status_code=302
                )
        ...
        return RedirectResponse(
            url=f"/tenants?success=tenant_updated",
            status_code=302
        )
        
    except Exception as e:
        db.rollback()
        logger.error(f"Edit tenant error: {str(e)}")
        return RedirectResponse(
            url=f"/tenants/{tenant_id}/edit?error=update_failed",
            status_code=302
        )
...
async def import_tenants_submit(
    ...
    try:
        if not file.filename.endswith('.csv'):
            return RedirectResponse(
                url="/tenants/import?error=invalid_file",
                status_code=302
            )
        ...
        for index, row in df.iterrows():
            try:
                ...
                existing_tenant = db.query(Tenant).filter(
                    Tenant.contact == str(row['Contact'])
                ).first()
                
                if existing_tenant:
                    errors.append(f"Row {index + 2}: Phone {row['Contact']} already exists")
                    continue
                ...
                tenant = Tenant(
                    ...
                    rent=int(float(row.get('Rent', 0))) if pd.notna(row.get('Rent')) and row.get('Rent') != 0 else None,
                    due_date=str(row.get('Due Date', '')).strip() if pd.notna(row.get('Due Date')) else None,
        ...
        return RedirectResponse(
            url=f"/tenants?success=imported_{imported_count}_tenants",
            status_code=302
        )
        
    except Exception as e:
        db.rollback()
        logger.error(f"Import tenants error: {str(e)}")
        return RedirectResponse(
            url="/tenants/import?error=import_failed",
            status_code=302
        )
...
async def _get_tenant_stats(db: Session) -> dict:
    ...
    try:
        ...
        late_fee_tenants = db.query(Tenant).filter(
            Tenant.active == True,
