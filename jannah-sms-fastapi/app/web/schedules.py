...
async def schedules_list(
    ...
    try:
        ...
        schedules = query.order_by(Schedule.created_at.desc()).offset(offset).limit(per_page).all()
        
        # Format schedules for display
        formatted_schedules = []
        for schedule in schedules:
            # Handle status field - it might be string or enum
            status_value = schedule.status.value if hasattr(schedule.status, 'value') else schedule.status
            formatted_schedules.append({
                ...
                "schedule_display": _format_schedule_display(_build_schedule_config_from_db(schedule)),
        ...
        recent_schedules = db.query(Schedule).filter(
            Schedule.last_run.isnot(None)
        ).order_by(Schedule.last_run.desc()).limit(5).all()
        ...
        logger.error(f"Schedules list error: {str(e)}")
        return templates.TemplateResponse(
            "error.html",
            {
                "request": request,
...
async def new_schedule_form(request: Request):
    ...
    try:
        ...
        logger.error(f"New schedule form error: {str(e)}")
        return templates.TemplateResponse(
            "error.html",
            {
                "request": request,
                "error": "Failed to load schedule form"
            }
        )
...
async def new_schedule_submit(
    ...
    try:
        ...
        schedule_config = _build_schedule_config(schedule_frequency, schedule_hour, schedule_minute)
        ...
        return RedirectResponse(
            url="/schedules?success=schedule_created",
            status_code=302
        )
        ...
        return RedirectResponse(
            url="/schedules/new?error=create_failed",
            status_code=302
        )
...
async def edit_schedule_form(schedule_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not schedule:
            return RedirectResponse(url="/schedules?error=schedule_not_found", status_code=302)
        ...
        logger.error(f"Edit schedule form error: {str(e)}")
        return templates.TemplateResponse(
            "error.html",
            {
                "request": request,
                "error": "Failed to load schedule edit form"
            }
        )
...
async def edit_schedule_submit(
    ...
    try:
        ...
        return RedirectResponse(
            url="/schedules?success=schedule_updated",
            status_code=302
        )
        ...
        return RedirectResponse(
            url=f"/schedules/{schedule_id}/edit?error=update_failed",
            status_code=302
        )


@router.get("/{schedule_id}", response_class=HTMLResponse)
async def schedule_details(schedule_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        formatted_schedule = {
            ...
            "conditions_summary": condition_service._summarize_conditions(schedule.conditions or {}),
            "next_run": schedule.next_run,
            "last_run": schedule.last_run,
            "execution_count": getattr(schedule, 'execution_count', schedule.run_count) or 0,
        ...
        logger.error(f"Schedule details error: {str(e)}")
        return templates.TemplateResponse(
            "error.html",
            {
                "request": request,
...
async def _get_schedule_stats(db: Session) -> dict:
    ...
    try:
        ...
        paused_schedules = db.query(Schedule).filter(
    ...
    try:
        ...
        if schedule_type == "daily" and ":" in schedule_value:
            ...
            if len(parts) >= 2:
                ...
                if ":" in time_part:
                    hour, minute = time_part.split(":")
                    config.update({"day": day, "hour": int(hour), "minute": int(minute)})
